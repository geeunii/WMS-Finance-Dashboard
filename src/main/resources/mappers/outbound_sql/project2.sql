...
ALTER TABLE outboundRequest MODIFY staff_id BIGINT NULL;
      ...
      -- 출고요청 테이블 (sr)
select sr.ShipmentRequestID, sr.ShipmentDate, sr.approvedStatus, sr.shipmentAddress,
       sr.member_Id, sr.warehouse_Id, sr.staff_Id,

       -- 출고품목 테이블 (si)
       si.shipment_item_id, si.product_Id, si.shipmentQuantity

from shipmentRequest sr
inner join shipmentItem si
on sr.shipmentRequestId = si.shipmentRequestId
where si.shipmentRequestId = 1
and sr.member_Id = 1;
...
WHERE
  ...
  AND so.approvedOrderID = 1      -- 특정 출고지시서 ID 필터
GROUP BY
    so.approvedOrderID, Pd.product_name;
